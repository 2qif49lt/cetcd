...
cetcd_watcher *cetcd_watcher_create(cetcd_client *cli, const char *key, uint64_t index,
	...
	parser =(struct cetcd_response_parser_t *)calloc(1, sizeof(cetcd_response_parser_t));

	parser->st = 0;
	parser->buf = sdsempty();
	parser->resp = calloc(1, sizeof(cetcd_response));

	watcher->parser = parser;
...
void cetcd_watcher_release(cetcd_watcher *watcher) {
    if (watcher) {
        ...
        if (watcher->parser) {
			struct cetcd_response_parser_t * parser =(struct cetcd_response_parser_t *) watcher->parser;
            sdsfree(parser->buf);
            if (parser->json) {
            ...
            cetcd_response_release((cetcd_response*)parser->resp);
            free(watcher->parser);
        }
        free(watcher);
    }
    ...
    while ((msg = curl_multi_info_read(mcurl, &ignore)) != NULL) {
        if (msg->msg == CURLMSG_DONE) {
            ...
            if (watcher->callback) {
                ...
                parser->resp = NULL; /*surpress it be freed again by cetcd_watcher_release*/
            }
            if (!watcher->once) {
                curl_multi_remove_handle(mcurl, curl);
...
#ifdef  _WIN32
static DWORD WINAPI cetcd_multi_watch_wrapper(void* args){
#else
...
int cetcd_multi_watch_async_stop(cetcd_client *cli, cetcd_watch_id wid) {
	...
	TerminateThread((HANDLE)wid,1);
	WaitForSingleObject((HANDLE)wid,INFINITE);
#else
	pthread_cancel(wid);
	pthread_join(wid, 0);
#endif
